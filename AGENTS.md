...
4. **Static output only** - no backend, no server functions, no runtime APIs, no third-party origins at all (fonts self-hosted; no analytics, and `terms.toml` says so in writing).
